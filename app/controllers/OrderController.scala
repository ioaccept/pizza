  ...
  def addOrder: Action[AnyContent] = Action { implicit request =>
    orderForm.bindFromRequest.fold(
      ...
      userData => {
        ...
        val myOrder = new Order(-1, request.session.get("id").get.toLong, request.session.get("user").get, request.session.get("distance").get.toLong, item.name, extras.name, userData.itemQuantity, userData.itemSize, 1, 1, null)
        myOrder.getOrderPrice()

        if (item.catName == "Pizza") {
          myOrder.deliveryTimePizza
